...
class MDepartemenController extends Controller
{
    public function index(Request $request) {
        ...
        try{
            ...
            return $this->respond($country, 'berhasil menampilkan data!', 200);
    ...
    public function store(Request $request) {
        ...
        DB::beginTransaction();
        try {
            $response = MDepartemen::create([ 'name' => $request->name ]);
            DB::commit();
    ...
    public function show($id) {
        try{
            $response = MDepartemen::with('jabatans', 'bagians')->findOrFail($id);
